...
# 연도와 분기 분리
restaurant_data['연도'] = restaurant_data['기준_년분기_코드'].astype(str).str[:-1].astype(int)
restaurant_data['분기'] = restaurant_data['기준_년분기_코드'].astype(str).str[-1:].astype(int)

# 분리된 시간 특성을 범주형으로 추가
categorical_features.extend(["연도", "분기"])
...
if hasattr(cat_encoder, 'get_feature_names_out'):
    ...
    for cat_feature in categorical_features:
        unique_cats = X[cat_feature].unique()
        for cat in unique_cats:
            transformed_feature_names.append(f"{cat_feature}_{cat}")
...
importance_df = pd.DataFrame({
    'feature': transformed_feature_names[:len(rf_importances)],
